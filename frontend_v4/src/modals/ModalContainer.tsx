...
const ModalContainer: React.FC<{ modalContents: IModalContents }> = (props) => {
  const {
    type,
    icon,
    iconScaleEffect,
  ...
  return (
    <>
      ...
      <ModalContainerStyled onClick={type === "noBtn" ? closeModal : undefined}>
        {/* {icon && (
...
const ModalIconImgStyled = styled.img<{ iconScaleEffect: boolean | undefined }>`
  width: ${(props) => (props.iconScaleEffect ? "95px" : "70px")};
  margin-bottom: 20px;
  ${(props) =>
